<?php 
	...
	if (isset($_POST['nameInput']) && $_POST['todo'] == 'getResult') {
		...
		$qry = "SELECT employees.emp_no as no, concat(first_name, ' ', last_name) as name, dept_name, title, format(salary, '##,##') as salary
				from employees 
		        ...
		        join (
					select emp_no, max(to_date) as max_todate 
					from salaries
					group by emp_no
		        ) saltemp
		        on salaries.emp_no=saltemp.emp_no and salaries.to_date = saltemp.max_todate 
